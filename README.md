...
A hands-on project for building an AI Agent from scratch. Iterate step by step — from a minimal CLI skeleton to conversational AI, function calling, MCP protocol, task management, and a main/sub-agent split — to create an extensible command-line AI assistant.
...
Every mode supports `/exit`, `/clear`, `/help`. `task` adds `/status`; `subagent` adds `/agents` and `/logs` for tracing delegations.
...
```
CLI Skeleton → AI Chat → Function Call → MCP Protocol → Task Mgmt → SubAgent → Multi-Agent (next)
 Command         Streaming   Tool dispatch   Client-Server  State machine  Main/Sub   Agent comm network
 parsing         Multi-turn  Dual-mode CLI   External tools Smart routing  delegation
```
