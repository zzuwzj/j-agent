...
从零构建 AI Agent 的实战项目。逐步迭代，从 CLI 骨架到对话能力、工具调用、MCP 协议、任务管理、主从 Agent 分工、按需加载的 Skills 知识库、多 Agent 团队协作、跨会话持久化记忆，打造一个可扩展的命令行 AI 助手。
...
- Memory：基于文件的持久化记忆（用户画像 / 项目上下文 / 决策记录），中间件自动注入背景 + LLM 主动写入工具
...
npx j-agent memory     # Memory 模式（跨会话记住偏好、项目背景与决策）
```

每种模式都支持 `/exit`、`/clear`、`/help`；`task` 额外有 `/status`；`subagent` 额外有 `/agents`、`/logs`；`skills` 额外有 `/skills`、`/stats`、`/reset-cache`；`team` 额外有 `/agents`、`/stats`、`/messages`、`/verbose`；`memory` 额外有 `/memory`、`/sessions`、`/switch-user`、`/switch-project`、`/forget-project`、`/cleanup`。
...
| [Day 9](docs/zh-CN/day9.md) | Agent 持久化与记忆 | 文件存储、记忆中间件、显式/隐式记忆、多用户隔离 |
...
```
CLI → AI 聊天 → 工具调用 → MCP → 任务管理 → SubAgent → Skills → Agent Team → 记忆 → 安全 (下一步)
