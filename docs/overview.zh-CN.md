...
```
Day 1: CLI 骨架 → Day 2: AI 聊天 → Day 3: 工具调用 → Day 4: MCP 协议 → Day 5: 任务管理 → Day 6+: 多 Agent
  命令解析         流式对话         Function Call      MCP Client-Server  任务状态机       Agent 协作
  目录结构         多轮上下文        双模式交互         外部服务集成        智能路由         任务分配聚合
```