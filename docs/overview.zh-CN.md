...
```
Day 1 CLI 骨架 → Day 2 AI 聊天 → Day 3 工具调用 → Day 4 MCP → Day 5 任务管理 → Day 6 SubAgent → Day 7 Skills → Day 8+ 记忆
 命令解析         流式对话         Function Call    Client-Server 状态机         主从委托         领域知识按需     持久化与偏好
```